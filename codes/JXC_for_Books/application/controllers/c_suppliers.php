...
class c_suppliers extends CI_Controller {
  ...
  */
  public function addSupplier()
  {
    ...
    $query = $this -> m_suppliers -> checkSuppliersByName($supName);
    if($query->num_rows() == 0)
    {
      $this -> m_suppliers -> addSupplier($supName, $supArea, $supPinyin, $supBrief, $supTel, $supContact, $supFax, $supPostcode, $supAddress, $supBank, $supBankAccount, $supEmail, $supInternet, $supNote);
      $data = array('msg' => 'true');
    }
  ...
  */
  public function delSupplier()
  {
    ...
    $query = $this -> m_suppliers -> checkSuppliersByID($supID);
    if($query -> num_rows() != 0)
    {
  ...
  */
  ...
  public function changeSupplier()
  {
    ...
    if($this -> m_suppliers -> changeSupplier($supID, $supName, $supPinyin, $supTel, $supContact,$supFax, $supPostcode, $supAddress, $supBank, $supBankAccount, $supEmail, $supInternet, $supNote)) {
      $data = array('msg' => "true");
    }
    else{
      $data = array('msg' => "false");
